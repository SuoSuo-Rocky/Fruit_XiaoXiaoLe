...
class Manager(Base):
    ...
    游戏进行中的管理
    """
    # 计分规则列表:  消除每一类小动物所得的分数
    death_sign =  True                       # 死图标志，初始不是死图
    every_animal_score = [1, 2, 1, 3, 2, 1]  # 上一次 选中的小动物, 值为小动物索引
    destory_animal_num = [0, 0, 0, 0, 0, 0]  # 消除动物列表： 消除各小动物的个数
    ...
    def start_game_init(self):
        ...
        # score_board.image = pygame.transform.smoothscale(score_board.image, (170, 170))
        score_board.draw(self.screen)
        # 绘制  游戏 倒计时 图片
        # Element(Element.count_down_image, Element.count_down_posi).draw(self.screen)
        # 绘制 游戏分数
        ...
        if event.type == MOUSEBUTTONDOWN: # 鼠标按下事件
            ...
            if self.status == 1:
                ...
                if self.matrix_topleft[0] < mouse_x < self.matrix_topleft[0] + self._cell_size * self._width \
                    ...
                    # 判断前后点击的两个小动物是否相邻
                    if (self.last_sel[0] == self.cur_sel[0] and abs(self.last_sel[1] - self.cur_sel[1]) == 1) \
                            or (self.last_sel[1] == self.cur_sel[1] and abs(self.last_sel[0] - self.cur_sel[0]) == 1):
                ...
                # 判断点击的是退出按钮, 需注意此退出按钮的坐标为左上角
                    ...
                    # 不可为  self.status  = 2, 不理解请移步 python 面向对象章节
                    Base.status = 2
    ...
    def exist_right(self, row, col, num):
        """ 判断 self.animal[i][j] 元素  右边  是否 存在 与其自身 图像 相同的 num - 1 个  图像 """
    ...
    def exist_down(self, row, col, num):
        """ 判断 self.animal[i][j] 元素  下方  是否 存在 与其自身 图像 相同的 num - 1 个  图像 """
    ...
    def exist_left(self, row, col, num):
        """ 判断 self.animal[i][j] 元素  左方  是否 存在 与其自身 图像 相同的 num - 1 个  图像 """
        if col >= num - 1:
            for item in range(num):
                if self.animal[row][col] != self.animal[row][col - item] or self.animal[row][col] == -2:
                    break
    ...
    def change_right(self, row, col, num):
        """ 改变 当前小动物及 右边的 num 个小动物 为 消除 状态 """
        for item in range(num):
    ...
    def stop_game(self):
        ...
        if self.status == 1:
            ...
            if self.time_is_over:
                pygame.time.delay(1000)  # 暂停程序一段时间
                # 绘制 "时间到了" 图片
                Element(Element.time_is_over_image, Element.time_is_over_posi).draw(self.screen)
                pygame.display.flip()
                pygame.time.delay(2000)   # 暂停程序一段时间, 避免 "game over" 图片 一闪而过.
    ...
    def judge_time(self):
        ...
        # 避免 在 self.status = 2 的 情况下更新 self.end_time , 从而使 self.time_is_over == True
        if self.status == 1:
    ...
    def mouse_image(self):
        ...
        mouse_x -= 10
        mouse_y -= 10
        Element(Element.mouse_replace_image, (mouse_x, mouse_y)).draw(self.screen)
        pygame.display.flip()  # 不 可 忽略 ，
